module Malvin.Gossip.Caas where

import Malvin.Gossip
import qualified Data.IntSet as IntSet
import qualified Data.IntMap as IntMap
import qualified Data.IntMap.Strict as IntMap.Strict (filterWithKey)
...
subGraphWith (nRel,sRel) ys = (fix nRel, fix sRel) where
  ...
  fix r = IntMap.map (IntSet.intersection subset) $ restrictKeys' r subset

restrictKeys' :: IntMap.IntMap a -> IntSet.IntSet -> IntMap.IntMap a
restrictKeys' m s = IntMap.Strict.filterWithKey (\k _ -> k `IntSet.member` s) m
