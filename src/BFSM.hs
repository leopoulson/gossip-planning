...
doBFS :: Eq a => FSM ch a -> Maybe [a]
doBFS fsm = bfs fsm (map bNode $ initial fsm) []

bfs :: Eq a => FSM ch a -> [BNode a ch] -> [a] -> Maybe [a]
...
rebuildPath :: BNode a ch -> [a]
rebuildPath (BNode a (Just (bn, ch))) = a : rebuildPath bn
rebuildPath (BNode a Nothing)   = [a]
