...
** Ch 2 - Background
*** TODO Move Uniform Strategies stuff to the appendix ? 
** Ch 3 - Algorithm
*** TODO Consider changing the examples in §3. 
    Using K_A (Sbc) and then K_a (~Sba) is a bit annoying, could change it to be
    just the same the whole way through? 
*** TODO Need to make changes to §5 searching. 
*** TODO Maybe change first paragraph in MEStar ? 
*** TODO Address the way we draw diagrams of PSAs. 
    Namely, the fact that we just draw the state S. 
*** DONE Add examples to introduction. 
    CLOSED: [2019-05-08 Wed 11:37]
** Ch 4 - Implementation
*** TODO Add link to source code at start of implementation. 
*** TODO Fix formatting, figures overrunning
*** TODO Address code - compacting and removing stuff 
*** TODO Refer back to benefits of Haskell each time
**** Model
...
*** TODO Add detail to the profiling - that we take averages 
